-- Setup
CREATE EXTERNAL SCHEMA silver
FROM DATA CATALOG
DATABASE 'silver_db'
IAM_ROLE 'your-iam-role'
REGION 'eu-north-1';

CREATE EXTERNAL SCHEMA bronze
FROM DATA CATALOG
DATABASE 'bronze_db'
IAM_ROLE 'your-iam-role'
REGION 'eu-north-1';

CREATE SCHEMA IF NOT EXISTS gold;

-- Check if you can access silver layer tables
SELECT * FROM silver.crm_cust_info LIMIT 2;

-- Create Views for the gold layer

-- dim_customers view
DROP VIEW IF EXISTS gold.dim_customers;
CREATE OR REPLACE VIEW gold.dim_customers AS
SELECT
  ROW_NUMBER() OVER (ORDER BY ci.cst_id) AS customer_key,
  ci.cst_id AS customer_id,
  ci.cst_key AS customer_number,
  ci.cst_firstname AS first_name,
  ci.cst_lastname AS last_name,
  ci.cst_marital_status AS marital_status,
  CASE WHEN ci.cst_gndr <> 'n/a' THEN ci.cst_gndr
       ELSE COALESCE(ca.gen, 'n/a') END AS cust_gender,
  ca.bdate AS birthdate,
  cl.country AS country,
  ci.cst_create_date AS create_date
FROM silver.crm_cust_info ci
LEFT JOIN silver.erp_cust_az12 ca ON ci.cst_key = ca.cid
LEFT JOIN silver.erp_loc_a101 cl ON ci.cst_key = cl.cid
WITH NO SCHEMA BINDING;

-- dim_products view
DROP VIEW IF EXISTS gold.dim_products;
CREATE OR REPLACE VIEW gold.dim_products AS
SELECT
  ROW_NUMBER() OVER (ORDER BY pi.prd_start_dt, pi.prd_key) AS product_key,
  pi.prd_id AS product_id,
  pi.prd_key AS product_number,
  pi.prd_nm AS product_name,
  pi.cat_id AS category_id,
  pc.cat AS category,
  pc.subcat AS subcategory,
  pi.prd_line AS product_line,
  pi.prd_cost AS cost,
  pi.prd_start_dt AS product_start_date,
  pc.maintenance AS maintenance
FROM silver.crm_prd_info pi
LEFT JOIN silver.erp_px_cat_g1v2 pc ON pi.cat_id = pc.id
WHERE pi.prd_end_dt IS NULL
WITH NO SCHEMA BINDING;

-- fact_sales view
DROP VIEW IF EXISTS gold.fact_sales;
CREATE OR REPLACE VIEW gold.fact_sales AS
SELECT
  sd.sls_ord_num AS order_number,
  c.customer_key,
  p.product_key,
  sd.sls_order_dt AS order_date,
  sd.sls_ship_dt  AS ship_date,
  sd.sls_due_dt   AS due_date,
  sd.sls_sales     AS sales_amount,
  sd.sls_quantity   AS quantity,
  sd.sls_price      AS price
FROM silver.crm_sales_details sd
LEFT JOIN gold.dim_customers c ON sd.sls_cust_id = c.customer_id
LEFT JOIN gold.dim_products p ON sd.sls_prd_key = p.product_number
WITH NO SCHEMA BINDING;

--aggregation's example
SELECT country ,SUM(sales_amount) as total_sales FROM
gold.fact_sales f INNER JOIN 
gold.dim_customers c ON f.customer_key=c.customer_id
GROUP BY country;